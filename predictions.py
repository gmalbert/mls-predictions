...
try:
    from models.ev_engine import compute_team_rankings
    from models.market_predictions import get_team_xg_averages
...
@st.cache_data(ttl=3600)
def compute_league_stats(csv_path: str, season: int) -> dict | None:
    """Compute MLS base-rate statistics for a given season from historical data."""
...
# ── Entry point ───────────────────────────────────────────────────────────────
def home_page() -> None:
    """Main predictions page — 5-tab layout."""
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    with tab1:
        # ── League season stats banner ─────────────────────────────────────────────
        current_season = datetime.now().year
        league_stats = compute_league_stats(csv_path, current_season)
        if league_stats:
            with st.expander(
                f"📈 MLS {current_season} Season Stats ({league_stats['n']} matches played)",
                expanded=False,
            ):
                ...
                g1.metric("Over 1.5", f"{league_stats['over_1_5_pct']:.0%}")
                g2.metric("Over 3.5", f"{league_stats['over_3_5_pct']:.0%}")
                g3.metric("Clean Sheets", f"{league_stats['clean_sheet_pct']:.0%}")
    ...
    with tab3:
        ...
        if not ASA_AVAILABLE:
            ...
            if team_xg.empty:
                ...
                # ── Team rankings table (attack + defense 1-N) ─────────────────
                if MODELS_AVAILABLE:
                    st.markdown("#### 🏆 Offense & Defense Rankings")
                    try:
                        rankings_df = compute_team_rankings(team_xg)
                        if not rankings_df.empty:
                            n = len(rankings_df)
                            top5, bot5 = 5, n - 4

                            def _rank_style(val: int) -> str:
                                if val <= top5:
                            ...
                            rankings_display = rankings_df.copy()
                            rankings_display.columns = [
                                "Team", "GP", "xG/Game", "xGA/Game", "Attack Rank", "Defense Rank"
                            ]
                            styled_r = rankings_display.style.map(
                                _rank_style, subset=["Attack Rank", "Defense Rank"]
                            ).format({"xG/Game": "{:.3f}", "xGA/Game": "{:.3f}"})
                            st.dataframe(
                                styled_r,
                                hide_index=True,
                                height=get_dataframe_height(rankings_display, max_height=500),
                            )
                            st.caption(
                                "Attack Rank #1 = most xG per game (best offence). "
                                "Defense Rank #1 = fewest xGA per game (best defence). "
                                "🟢 Top 5 · 🔴 Bottom 5."
                            )
                    except Exception:
                        pass
...
# ── Sidebar logo (shown on every page) ───────────────────────────────────────
_logo_path = path.join(DATA_DIR, "logo.png")
...
pg = st.navigation(
    {
        "": [
            st.Page(home_page, title="MLS Predictor", icon="⚽", default=True),
        ],
        "Markets": [
